# Regime 5 — Momentum Crash Regime
...
Momentum Crash Regime — momentum factor reverses violently after crisis or extreme crowding
...
### `R05-S01` — Stress mean reversion (tail hedge tilt) — Momentum Crash Regime

Fade local extremes in Momentum Crash Regime when liquidity premia widen; institutional macro overlay book pattern.
...
### `R05-S02` — Wide-stop directional breakout — Momentum Crash Regime

Trade expansion after compression in Momentum Crash Regime with widened stops (prop desks reduce size, widen λ).
...
### `R05-S03` — Crash / momentum reversal scalp — Momentum Crash Regime

Short-term reversal after sharp impulsive leg in Momentum Crash Regime (crash risk literature; tactical desks).

**References:** Daniel & Moskowitz (2016) momentum crashes (JFE) — Cooper, Gutierrez & Hameed (2004) 'market states' & reversals

### `R05-S04` — Trend-with-vol overlay (short side bias ready) — Momentum Crash Regime

Directional trade aligned to dominant trend in Momentum Crash Regime but only after vol confirms participation.
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q2 — Trend High Volatility/regimes/r05.md` when this ID is in that quadrant’s list.
