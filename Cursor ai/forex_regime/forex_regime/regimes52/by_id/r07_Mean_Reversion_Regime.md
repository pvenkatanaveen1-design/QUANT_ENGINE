# Regime 7 — Mean Reversion Regime
...
Mean Reversion Regime — price ~2σ from mean; statistical reversion under stationarity / bounded range
...
### `R07-S01` — Range fade (upper boundary) — Mean Reversion Regime

Fade local premium in Mean Reversion Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R07-S02` — Range fade (lower boundary) — Mean Reversion Regime

Buy discount in Mean Reversion Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R07-S03` — Z-score oscillation — Mean Reversion Regime

Enter on standardized deviation from local mean under Mean Reversion Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R07-S04` — Premium/discount equilibrium trade — Mean Reversion Regime

Scale toward 50% dealing range equilibrium in Mean Reversion Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r07.md` when this ID is in that quadrant’s list.
