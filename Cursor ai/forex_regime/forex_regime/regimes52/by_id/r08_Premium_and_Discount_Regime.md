# Regime 8 — Premium and Discount Regime
...
Premium and Discount Regime — price vs ~50% equilibrium of dealing range (ICT-style range anatomy)
...
### `R08-S01` — Range fade (upper boundary) — Premium and Discount Regime

Fade local premium in Premium and Discount Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R08-S02` — Range fade (lower boundary) — Premium and Discount Regime

Buy discount in Premium and Discount Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R08-S03` — Z-score oscillation — Premium and Discount Regime

Enter on standardized deviation from local mean under Premium and Discount Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R08-S04` — Premium/discount equilibrium trade — Premium and Discount Regime

Scale toward 50% dealing range equilibrium in Premium and Discount Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r08.md` when this ID is in that quadrant’s list.
