# Regime 12 — Distribution (Wyckoff)
...
Distribution — institutions distributing into strength at highs; UTAD narrative
...
### `R12-S01` — Range fade (upper boundary) — Distribution (Wyckoff)

Fade local premium in Distribution (Wyckoff) toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R12-S02` — Range fade (lower boundary) — Distribution (Wyckoff)

Buy discount in Distribution (Wyckoff) when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R12-S03` — Z-score oscillation — Distribution (Wyckoff)

Enter on standardized deviation from local mean under Distribution (Wyckoff); pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R12-S04` — Premium/discount equilibrium trade — Distribution (Wyckoff)

Scale toward 50% dealing range equilibrium in Distribution (Wyckoff) (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r12.md` when this ID is in that quadrant’s list.
