...
Reflation Regime — recovery from deflationary scare; cyclicals/commodities leadership phase
...
### `R22-S01` — TSMOM / vol-managed pullback — Reflation Regime

Buy structural pullbacks while higher-timeframe trend holds in Reflation Regime; size down when realized variance spikes.

**References:** Moskowitz, Ooi & Pedersen (2012) 'Time Series Momentum' (JFE) — Barroso & Santa-Clara (2015) 'Momentum Has Its Moments' (JF)

### `R22-S02` — Channel / Donchian breakout — Reflation Regime

Enter on range expansion in the direction of Reflation Regime (institutional trend desks, managed futures CTAs).

**References:** Classic Donchian / Turtle-style breakout trend following — Krausz (1997) channel + MA hybrid literature

### `R22-S03` — Cross-sectional momentum add-on — Reflation Regime

Add on strong continuation closes when Reflation Regime supports risk appetite and breadth.
...
### `R22-S04` — Low-vol trend drift (defensive gross-up) — Reflation Regime

Hold / pyramid slowly in Reflation Regime; emphasis on not crowding into identical payoff tails.
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q1 — Trend Low Volatility/regimes/r22.md` when this ID is in that quadrant’s list.
