...
Fear Regime — VIX elevated; forced deleveraging, liquidity premiums
...
### `R26-S01` — Liquidity sweep fade — Fear Regime

Fade engineered wicks / false breaks consistent with Fear Regime (order flow narration; test empirically).

**References:** Evans & Lyons microstructure order flow — Hasbrouck (1991) information content of trades

### `R26-S02` — Vol-cluster straddle / flat gamma proxy fade — Fear Regime

Stand aside or trade only mean-reversion spikes in Fear Regime; vol desks reduce naked gamma.

**References:** Bollerslev (1986) GARCH / vol clustering — Barndorff-Nielsen & Shephard realized volatility

### `R26-S03` — Event reversal template — Fear Regime

Fade post-event overreaction when Fear Regime implies crowded positioning (news microstructure).

**References:** Brandt et al. post-earnings announcement drift vs reversal contexts — FX fix / macro surprise literature

### `R26-S04` — Regime-switch cautious drift — Fear Regime

Small size carry until Markov / HMM-style state probabilities stabilise in Fear Regime.
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q4 — Transition or Chaos/regimes/r26.md` when this ID is in that quadrant’s list.
