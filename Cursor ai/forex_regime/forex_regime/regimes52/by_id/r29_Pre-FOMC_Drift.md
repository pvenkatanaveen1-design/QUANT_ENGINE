...
Pre-FOMC Drift — scheduled-event risk premium / positioning mechanics (mixed evidence, context-specific)
...
### `R29-S01` — Range fade (upper boundary) — Pre-FOMC Drift

Fade local premium in Pre-FOMC Drift toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R29-S02` — Range fade (lower boundary) — Pre-FOMC Drift

Buy discount in Pre-FOMC Drift when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R29-S03` — Z-score oscillation — Pre-FOMC Drift

Enter on standardized deviation from local mean under Pre-FOMC Drift; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R29-S04` — Premium/discount equilibrium trade — Pre-FOMC Drift

Scale toward 50% dealing range equilibrium in Pre-FOMC Drift (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r29.md` when this ID is in that quadrant’s list.
