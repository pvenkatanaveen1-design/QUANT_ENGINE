# Regime 33 — Options Expiry Regime
...
Options Expiry Regime — gamma / OI pinning; dealer hedging flows near strikes
...
### `R33-S01` — Range fade (upper boundary) — Options Expiry Regime

Fade local premium in Options Expiry Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R33-S02` — Range fade (lower boundary) — Options Expiry Regime

Buy discount in Options Expiry Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R33-S03` — Z-score oscillation — Options Expiry Regime

Enter on standardized deviation from local mean under Options Expiry Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R33-S04` — Premium/discount equilibrium trade — Options Expiry Regime

Scale toward 50% dealing range equilibrium in Options Expiry Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r33.md` when this ID is in that quadrant’s list.
