...
Asian Range Regime — overnight balance; London/NY breakout games
...
### `R37-S01` — Range fade (upper boundary) — Asian Range Regime

Fade local premium in Asian Range Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R37-S02` — Range fade (lower boundary) — Asian Range Regime

Buy discount in Asian Range Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R37-S03` — Z-score oscillation — Asian Range Regime

Enter on standardized deviation from local mean under Asian Range Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R37-S04` — Premium/discount equilibrium trade — Asian Range Regime

Scale toward 50% dealing range equilibrium in Asian Range Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r37.md` when this ID is in that quadrant’s list.
