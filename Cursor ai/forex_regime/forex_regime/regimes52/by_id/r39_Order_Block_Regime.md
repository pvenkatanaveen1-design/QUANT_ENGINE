...
Order Block Regime — return to defended auction zones; reaction trade framework
...
### `R39-S01` — Range fade (upper boundary) — Order Block Regime

Fade local premium in Order Block Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R39-S02` — Range fade (lower boundary) — Order Block Regime

Buy discount in Order Block Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R39-S03` — Z-score oscillation — Order Block Regime

Enter on standardized deviation from local mean under Order Block Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R39-S04` — Premium/discount equilibrium trade — Order Block Regime

Scale toward 50% dealing range equilibrium in Order Block Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r39.md` when this ID is in that quadrant’s list.
