# Regime 40 — Imbalance / FVG Regime
...
Imbalance / FVG Regime — gap/inefficiency fill before continuation (practitioner)
...
### `R40-S01` — Range fade (upper boundary) — Imbalance / FVG Regime

Fade local premium in Imbalance / FVG Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R40-S02` — Range fade (lower boundary) — Imbalance / FVG Regime

Buy discount in Imbalance / FVG Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R40-S03` — Z-score oscillation — Imbalance / FVG Regime

Enter on standardized deviation from local mean under Imbalance / FVG Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R40-S04` — Premium/discount equilibrium trade — Imbalance / FVG Regime

Scale toward 50% dealing range equilibrium in Imbalance / FVG Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r40.md` when this ID is in that quadrant’s list.
