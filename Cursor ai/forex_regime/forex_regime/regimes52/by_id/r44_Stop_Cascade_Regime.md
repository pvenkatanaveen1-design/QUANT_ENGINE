...
Stop Cascade Regime — liquidity vacuum past clustered stops; snapback risk
...
### `R44-S01` — Stress mean reversion (tail hedge tilt) — Stop Cascade Regime

Fade local extremes in Stop Cascade Regime when liquidity premia widen; institutional macro overlay book pattern.
...
### `R44-S02` — Wide-stop directional breakout — Stop Cascade Regime

Trade expansion after compression in Stop Cascade Regime with widened stops (prop desks reduce size, widen λ).
...
### `R44-S03` — Crash / momentum reversal scalp — Stop Cascade Regime

Short-term reversal after sharp impulsive leg in Stop Cascade Regime (crash risk literature; tactical desks).

**References:** Daniel & Moskowitz (2016) momentum crashes (JFE) — Cooper, Gutierrez & Hameed (2004) 'market states' & reversals

### `R44-S04` — Trend-with-vol overlay (short side bias ready) — Stop Cascade Regime

Directional trade aligned to dominant trend in Stop Cascade Regime but only after vol confirms participation.
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q2 — Trend High Volatility/regimes/r44.md` when this ID is in that quadrant’s list.
