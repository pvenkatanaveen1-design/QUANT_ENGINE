...
Dollar Bear Regime — broad USD downtrend; supports gold/commodity complex mechanically
...
### `R50-S01` — TSMOM / vol-managed pullback — Dollar Bear Regime

Buy structural pullbacks while higher-timeframe trend holds in Dollar Bear Regime; size down when realized variance spikes.

**References:** Moskowitz, Ooi & Pedersen (2012) 'Time Series Momentum' (JFE) — Barroso & Santa-Clara (2015) 'Momentum Has Its Moments' (JF)

### `R50-S02` — Channel / Donchian breakout — Dollar Bear Regime

Enter on range expansion in the direction of Dollar Bear Regime (institutional trend desks, managed futures CTAs).

**References:** Classic Donchian / Turtle-style breakout trend following — Krausz (1997) channel + MA hybrid literature

### `R50-S03` — Cross-sectional momentum add-on — Dollar Bear Regime

Add on strong continuation closes when Dollar Bear Regime supports risk appetite and breadth.
...
### `R50-S04` — Low-vol trend drift (defensive gross-up) — Dollar Bear Regime

Hold / pyramid slowly in Dollar Bear Regime; emphasis on not crowding into identical payoff tails.
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q1 — Trend Low Volatility/regimes/r50.md` when this ID is in that quadrant’s list.
