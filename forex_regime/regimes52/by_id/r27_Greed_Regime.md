...
Greed Regime — volatility complacency; crowding, low risk premia
...
### `R27-S01` — Range fade (upper boundary) — Greed Regime

Fade local premium in Greed Regime toward equilibrium; stat-arb / STIR desk style mean reversion.

**References:** Avellaneda & Lee (2010) statistical arbitrage mean reversion — Potter & Bouchaud short-horizon MR in ranges

### `R27-S02` — Range fade (lower boundary) — Greed Regime

Buy discount in Greed Regime when process shows stationary range behavior.

**References:** Same MR foundations as upper fade — Band-trading practitioner risk: partial exits into midline

### `R27-S03` — Z-score oscillation — Greed Regime

Enter on standardized deviation from local mean under Greed Regime; pairs / basket desks analogue.

**References:** Pole & critic: Ornstein–Uhlenbeck-inspired OU trading heuristics — Gatev, Goetzmann & Rouwenhorst (2006) pairs trading

### `R27-S04` — Premium/discount equilibrium trade — Greed Regime

Scale toward 50% dealing range equilibrium in Greed Regime (ICT-style auction framing; discretionary).
...
If you ran `_write_regime_docs.py`, extra notes may exist under project folder `Q3 — Range Low Volatility/regimes/r27.md` when this ID is in that quadrant’s list.
